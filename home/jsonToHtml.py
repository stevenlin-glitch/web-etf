...
_ETF_DATES_PATH = "/var/www/html/web/etf/home/etf_dates.json"
try:
...
{ROWS}
    ...
    <!-- DEV ONLY: 模擬日期面板；prod 不提供模擬日期，整段停用
    <div id="debug-panel">
    ...
    <script>
        ...
        function openModal(etfId, etfName) {
            ...
            document.getElementById("sample-csv-link").href = `${DEV_API}/api/sample_csv/${etfId}`;
        ...
        document.getElementById("modal-upload-btn").addEventListener("click", async () => {
            ...
            try {
                const response = await fetch(`${DEV_API}/api/upload_csv`, {
                    method: "POST",
                    body: formData
                });
        ...
        // 記錄本次開啟頁面（IP / 時間 / 瀏覽器由後端 request 取得）
        // prod 專屬功能，dev 版沒有；從 dev 移植前端時勿漏掉這一段。
        (function () {
            try {
                fetch(`${DEV_API}/api/log_visit`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ page: 'etf_home', referrer: document.referrer }),
                    keepalive: true
                }).catch(() => {});
            } catch (e) {
                // 記錄失敗不影響頁面正常顯示
        ...
        async function refreshEtfDates() {
            try {
                const res = await fetch(`${DEV_API}/api/etf_dates`);
        ...
        async function refreshAdjustStatus() {
            try {
                const [timesRes, statusRes] = await Promise.all([
                    fetch(`${DEV_API}/api/upload_times`),
                    fetch(`${DEV_API}/api/etf_adjust_status`)
                ]);
        ...
        // ════ DEV: 模擬日期控制／寫入DB —— prod 停用（無對應介面與 API）════
        // // ════ DEV: 模擬日期控制 ════
        // // DEV_API 已於 <script> 開頭定義（依 http/https 自動切換）

        // async function loadMockDate() {
            // try {
                // const res = await fetch(`${DEV_API}/api/mock_date`);
                // const data = await res.json();
        ...
        // async function switchMockDate(dateVal, timeVal) {
            ...
            // try {
                // await fetch(`${DEV_API}/api/mock_date`, {
                    // method: 'POST',
                    // headers: { 'Content-Type': 'application/json' },
        ...
        // document.getElementById('debug-write-db-btn').addEventListener('click', async () => {
            ...
            // try {
                // const res = await fetch(`${DEV_API}/api/run_db_importer`, { method: 'POST' });
                // const data = await res.json();
        ...
        async function openReverseModal(etfId, etfName) {
            ...
            try {
                const res = await fetch(`${DEV_API}/api/reverse_csv_dates/${etfId}`);
                const data = await res.json();
        ...
        document.getElementById("reverse-download-btn").addEventListener("click", async () => {
            ...
            try {
                const res = await fetch(`${DEV_API}/api/reverse_csv/${currentReverseEtfId}?date=${dateVal}`);
        ...
        (function () {
            ...
            async function submitPeriod(method, payload) {
                ...
                try {
                    ...
                    const res = await fetch(`${DEV_API}/api/adjust_period/${currentPeriodEtfId}`, opts);
                    const data = await res.json();
...
# 執行
# cd /var/www/html/web/etf/home && /home/webuser/etf/venv/bin/python jsonToHtml.py