...
#debug-panel .debug-current span { font-weight: bold; }
...
#debug-panel .debug-apply-btn:hover { background: #d49200; }
...
#debug-panel .debug-clear-btn:hover { background: #777; }
...
#debug-panel .debug-write-db-btn:hover { background: #a93226; }
#debug-panel .debug-write-db-btn:disabled { background: #555; cursor: not-allowed; }
#debug-db-result {
    margin-top: 6px;
    font-size: 11px;
    max-height: 80px;
...
<!-- DEV ONLY: 模擬日期面板（右下角） -->
<div id="debug-panel">
    <div class="debug-title">&#9881; DEV 模擬日期</div>
...
async function loadMockDate() {
    try {
        ...
        if (data.mock_date) {
            el.textContent = `模擬 ${data.mock_date}`;
