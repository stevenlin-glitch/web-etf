...
ETF_DATES_PATH     = '/var/www/html/web/etf/home/etf_dates.json'
GENDATEDATA_SH     = '/home/webuser/etf/etf_calculator/dateData/genDateData.sh'

# 訪客紀錄（prod 專屬功能，dev 版沒有）：log/YYYY/MM/visit_log_YYYY-MM-DD.csv
VISIT_LOG_DIR      = '/var/www/html/web/etf/RESTful_API/log'
_visit_log_lock    = threading.Lock()
# 使用者手動覆蓋的調整期間；DAO.py 會在算完自動區間後套用這份覆蓋
USER_DATES_PATH    = '/home/webuser/etf/etf_calculator/report_generator/dateData/dates_update_by_user.json'
...
TRACKER_LOG_DIR = '/home/webuser/etf/etf_calculator/sitcRebalanceTracker/logs'
...
def get_today():
    ...
    prod 不提供模擬日期：模擬機制（_mock_today / _mock_time 及啟動時的還原）
    僅存在於 app_dev.py，這裡一律走真實時間。
    """
    return datetime.today().date()


def get_now():
    """回傳真實 now。prod 不提供模擬時間，理由同 get_today()。"""
    return datetime.now()
...
# prod 不提供模擬日期／時間，整支停用（前端也沒有對應介面）
#@app.route("/api/mock_date", methods=["GET"])
#def get_mock_date():
...
def get_client_ip():
    """取得訪客實際 IP；若經由反向代理，優先採用 X-Forwarded-For 第一段。"""
...
def get_visit_log_path(now):
    """回傳當日訪客紀錄檔路徑：log/YYYY/MM/visit_log_YYYY-MM-DD.csv（並建立所需目錄）。"""
    day_dir = os.path.join(VISIT_LOG_DIR, now.strftime('%Y'), now.strftime('%m'))
    os.makedirs(day_dir, exist_ok=True)
    return os.path.join(day_dir, f"visit_log_{now.strftime('%Y-%m-%d')}.csv")


@app.route("/api/log_visit", methods=["POST"])
def log_visit():
    """記錄一筆網頁開啟紀錄（IP、時間、瀏覽器資訊）至當日 CSV（log/YYYY/MM/visit_log_YYYY-MM-DD.csv）。

    prod 專屬功能，dev 版沒有；移植 app_dev.py 時勿覆蓋掉這一段。

    Body JSON:
      page      (str, optional): 頁面識別字串，預設 "unknown"。
      referrer  (str, optional): document.referrer。
    """
    body = request.get_json(silent=True) or {}
    page      = (body.get('page') or 'unknown')[:200]
    ...
    try:
        with _visit_log_lock:
            log_path = get_visit_log_path(now)
            is_new = not os.path.exists(log_path)
            with open(log_path, 'a', newline='', encoding='utf-8-sig') as f:
                writer = csv.writer(f)
                if is_new:
                    writer.writerow(['timestamp', 'ip', 'user_agent', 'referrer', 'page'])
                writer.writerow(row)
...
def _run_gendatedata(skip_trading_day_check=False):
    ...
    """
    args = [GENDATEDATA_SH] #, "--from-upload-times"] # prod 走真實日期，不能用 from-upload-times
...
# 以下兩支只服務模擬日期，prod 停用。
# 特別注意：絕對不要在 prod 寫入 upload_times.json 的 'today' 欄位——
# 那是 dev 用來記錄 mock date 的欄位，prod 的 genDateData.sh 讀到會誤判。
...
@app.route("/api/adjust_period/<etf_id>", methods=["POST"])
def set_adjust_period(etf_id):
    ...
    純粹當作「這筆覆蓋屬於哪一期」的到期標記，DAO.py 會在 today 超過它時自動清除。
    """
...
@app.route("/api/adjust_period/<etf_id>", methods=["DELETE"])
def clear_adjust_period(etf_id):
    """清除該 ETF 的調整期間覆蓋，回到 DAO.py 自動計算的結果。"""
    if etf_id not in TARGET_ETF_LIST:
        return jsonify({"status": "error", "message": f"未知的 ETF：{etf_id}"}), 400
...
@app.route("/api/reverse_csv_dates/<etf_id>", methods=["GET"])
...
@app.route("/api/reverse_csv/<etf_id>", methods=["GET"])
...
#@app.route("/api/run_db_importer", methods=["POST"])
#def run_db_importer():
...
#                "/home/webuser/etf/etf_calculator/etfDbImporter/etfDbImporter.py",
#                "--from-upload-times",
#            ],
#            capture_output=True,
#            text=True,
