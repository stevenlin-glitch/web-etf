...
REQUIRED_COLUMNS = {'etf_code', 'stock_code', 'stock_name', 'action', 'estimated_shares'}
VALID_ACTIONS = {'addition', 'deletion', 'weight'}

# Debug: 模擬日期（None = 使用系統真實時間）
_mock_today = None  # type: date | None  (from datetime module)

# 啟動時從 upload_times.json 還原 mock date（若有 'today' 欄位）
try:
...
def get_now():
    """回傳模擬日期對應的 datetime（debug 模式）或真實 now。"""
    if _mock_today is not None:
        real = datetime.now()
        return real.replace(year=_mock_today.year, month=_mock_today.month, day=_mock_today.day)
    return datetime.now()
...
def get_mock_date():
    """回傳目前設定的模擬日期；null 表示使用真實日期。"""
    global _mock_today
    return jsonify({"mock_date": str(_mock_today) if _mock_today is not None else None})
...
def _write_today_to_upload_times():
    """將 get_today() 的結果寫入 upload_times.json 的 'today' 欄位。"""
    try:
        if os.path.exists(UPLOAD_TIMES_PATH):
            with open(UPLOAD_TIMES_PATH, 'r', encoding='utf-8') as f:
                data = json.load(f)
...
def download_sample_csv(etf_id):
    ...
    writer.writerow([etf_code, '2892', '第一金', 'addition', '1000'])
    writer.writerow([etf_code, '1402', '遠東新', 'deletion', '-1000'])
    writer.writerow([etf_code, '2330', '台積電', 'weight', '0'])
