...
def get_upload_times():
    """回傳所有 ETF 的最後上傳時間（來自 upload_times.json）。
    檔案不存在或讀取失敗時回傳空物件，不阻斷前端顯示。
    """
    try:
...
def download_sample_csv(etf_id):
    ...
    writer.writerow(['etf_code', 'stock_code', 'stock_name', 'action'])
...
def upload_csv():
    ...
    try:
        ...
        effective_hint = f"CSV 資料將於 {effective_date} 7:10 後生效"
