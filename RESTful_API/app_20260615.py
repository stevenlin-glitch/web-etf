...
def get_upload_times():
    ...
    需同時滿足：今天在可上傳期間內（effective_date ~ adjust_end），
    ...
    for etf_id, upload_time in data.items():
        if etf_id not in periods:
            continue
        try:
            eff_date    = datetime.strptime(periods[etf_id]['effective_date'], '%Y-%m-%d').date()
            adj_end     = datetime.strptime(periods[etf_id]['adjust_end'],     '%Y-%m-%d').date()
            upload_date = datetime.strptime(upload_time, '%Y-%m-%d %H:%M:%S').date()
            if eff_date <= today <= adj_end and eff_date <= upload_date <= adj_end:
...
def get_etf_adjust_status():
    ...
    in_period 判斷條件：effective_date <= today <= adjust_end。
    uploaded_in_period 判斷條件：upload_time 落在 effective_date ~ adjust_end 區間內。
    ...
    for etf_id, info in periods.items():
        try:
            eff_date  = datetime.strptime(info['effective_date'], '%Y-%m-%d').date()
            adj_end   = datetime.strptime(info['adjust_end'],     '%Y-%m-%d').date()
            in_period = eff_date <= today <= adj_end
            ...
            if in_period and etf_id in upload_times:
                try:
                    ...
                    uploaded_in_period = eff_date <= upload_date <= adj_end
                except Exception:
                    pass
    ...
    try:
        ...
        for i, row in enumerate(reader, start=2):  # 第 1 行為標題，資料從第 2 行起
            ...
            if action not in VALID_ACTIONS:
                errs.append(f"無效的 action：{action}（須為 addition 或 deletion）")
            ...
            if not shares_raw:
                ...
                try:
                    ...
                    if shares_val == 0:
                        errs.append("estimated_shares 不可為 0")
